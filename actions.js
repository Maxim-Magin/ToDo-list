...
toDoList.addEventListener("click", checkDivButtons);
...
function countNoCompletedTasks() {
    const noCompletedTasks = Array.from(toDoList.children).filter((element) => element.firstElementChild.className.includes("no_checked") === true);
...
function manageArrowDown(e) {
    ...
    if (countNoCompletedTasks() === toDoList.childElementCount) {
        Array.from(toDoList.children).forEach((element) => checkStatusTask(element));
    ...
    // Если какой-то из тасков не завершён
        ...
        Array.from(toDoList.children).forEach((element) => makeCheckEverything(element));
...
function manageVisibilityClearCompleted() {
    ...
    buttonClearCompleted.style["display"] = toDoList.childElementCount - countNoCompletedTasks() !== 0 ? "block" : "none"
}
